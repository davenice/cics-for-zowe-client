...
export function getShowRegionLogs(treeview: TreeView<any>) {
    return commands.registerCommand("cics-extension-for-zowe.showRegionLogs", async (node) => {
        ...
        var selectedRegion = allSelectedNodes[0];
        //    var profile = node.parentSession.profile;
        // our profile is winmvs28.cicsex61 ... so probably we need the parent profile and then zosmf...?
        // or somehow we need to request zosmf profiles to try?
        const cache = ProfileManagement.getProfilesCache();
        // const profile = await cache.getDefaultProfile("zosmf"); // the default profile returns undefined...?
        const profiles = await cache.fetchAllProfilesByType("zosmf");
        const profileNames = profiles.map((value) => {
            return value.name;
        });

        const quickPickOptions: vscode.QuickPickOptions = {
            placeHolder: vscode.l10n.t("Select the profile to use to find the logs"),
            ignoreFocusOut: true,
            canPickMany: false,
        };
        var chosenProfileName = await Gui.showQuickPick(profileNames, quickPickOptions);
        commands.executeCommand("zowe.jobs.setJobSpool", chosenProfileName, selectedRegion.jobid);
    });
}
